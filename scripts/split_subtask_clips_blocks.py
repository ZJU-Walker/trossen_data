#!/usr/bin/env python3
"""Cut per-subtask video clips from episode mp4s using the block segment CSV.
...
So for each episode you get two clips:
  observe_human (the human pointing / robot still) and
  put_<color>_block_to_plate (the robot carrying the block).
Watch them to confirm the motion-onset boundary lands at the right moment.

Defaults to episodes 0,1,2 of both block datasets.
"""
...
DEFAULT_CSV = DATA_ROOT / "scripts" / "labels" / "subtask_segments_0528_blocks_auto.csv"
DEFAULT_OUT = DATA_ROOT / "scripts" / "clips" / "subtask_clips_blocks"
...
def main() -> int:
    ...
    parser.add_argument(
        "--episodes", nargs="+", default=["000000", "000001", "000002"],
        help="Episode ids (zero-padded) to cut.",
    )
    parser.add_argument(
        "--datasets", nargs="+",
        default=["0528_green_block_mem", "0528_yellow_block_mem"],
    )
    ...
    for row in selected:
        ...
        manifest.append({
            "clip": dst.name, "dataset": dataset, "episode_id": episode_id,
            "subtask": subtask, "start_frame": start_frame, "end_frame": end_frame,
            "expected_frames": expected, "actual_frames": actual, "fps": fps,
