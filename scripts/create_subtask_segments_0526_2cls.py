#!/usr/bin/env python3
"""
Create and validate 2-class gripper-STATE segment labels for the 2026-05-26
marker datasets.

This is the 2-label variant of create_subtask_segments_0526.py. Instead of the
3-subtask scheme (keep_open -> close -> keep_closed), every frame is labeled by
the gripper STATE, so each episode is split into exactly 2 segments:

  - data_robot_give_0526: open  -> close   (gripper closes to take the marker)
  - data_robot_pull_0526: close -> open    (gripper opens to release the marker)

The split boundary is the START of the detected gripper transition: the label
flips to the target state the moment the gripper begins moving.

  give: open  = [0 .. start-1],  close = [start .. end_of_episode]
  pull: close = [0 .. start-1],  open  = [start .. end_of_episode]

where `start` = detection.start_frame from the same transition detector used by
the 3-class script. Labels remain source=auto / needs_manual_review.
"""
...
DEFAULT_AUTO_OUT = Path("labels/subtask_segments_0526_2cls_auto.csv")
DEFAULT_SUMMARY_OUT = Path("labels/subtask_segments_0526_2cls_summary.csv")
...
# 2-class STATE sequence: (state_before_transition, state_after_transition).
# give: gripper starts open, ends closed -> open then close
# pull: gripper starts closed, ends open -> close then open
SEQUENCE_BY_DATASET = {
    "data_robot_give_0526": ("open", "close"),
    "data_robot_pull_0526": ("close", "open"),
}
...
def generate_rows(
    ...
    for dataset_root in dataset_roots:
        ...
        for ep in episodes:
            ...
            # 2-class STATE labels: boundary at the START of the transition.
            # before-state covers [0 .. start-1], after-state covers [start .. end].
            state_before, state_after = SEQUENCE_BY_DATASET[dataset]
            boundary = detection.start_frame
            # Clamp so both segments are non-empty within the episode.
            boundary = max(1, min(boundary, ep.length - 1))
            segments = [
            ...
            notes = (
                ...
                f"; transition={detection.start_frame}-{detection.end_frame}"
